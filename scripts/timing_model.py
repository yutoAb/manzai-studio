...
# 遷移タイプごとの gap_ms 事前分布（平均, 標準偏差）。
# 実演GT(実在漫才3組・私的検証・計32分)を pyannote で diarization して測定した実測に基づき更新:
#   交替gap 中央≈-76ms / overlap率≈50% / 正gap中央≈688ms / 90%ile≈3.3s（天丼の溜め）
#   → 実演はターンがほぼ間ゼロ＆高頻度のかぶせ。当初の経験則(+170/+300ms)は緩すぎた。
# (注: かぶせ深さの実測は長区間overlapのアーティファクトで不安定なため、研究的妥当値 -180ms を採用)
PRIORS = {
    "opening":        (0,    0),     # 1行目
    "boke2tsukkomi":  (40,   140),   # 実測: 中央ほぼ0・しばしば軽い重なり
    "tsukkomi2boke":  (140,  160),   # ボケが次を繰り出す前の軽い間
    "kabuse":         (-180, 90),    # 食い気味（負gap）・実演で高頻度
    "tame":           (650,  350),   # 天丼/転換前の溜め（正gap中央〜90%ile）
    "closing":        (250,  90),    # 締めの挨拶前
    "same_speaker":   (300,  150),   # 同一話者の連続
}
